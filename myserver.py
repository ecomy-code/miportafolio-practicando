...
app.config.from_mapping(SECRET_KEY='azby',DATABASE=os.path.join(app.instance_path, 'mysite.sqlite'),)
...
def create_app():
    ...
    def loginf():
    	...
    	if request.method == "POST":
    		...
    		time.sleep(2)
    		user = UsuariosOficiales.query.filter_by(username=request.form["username"]).first()
    		if user and check_password_hash(user.password, request.form["password"]):
    			...
    			nombre_usuario = usuario_condominio
    			...
    			usuarioCompleto = [[id_condominio, usuario_condominio,password_condominio,nombreusuario_condominio, codigo,celular, direccion ]]
    			time.sleep(1)
    			mensaje = ''
    			...
    			meritos='98'
    			amonestaciones='0'
    			return render_template('oficiales/index.html',nombre_xxx=nombre_xxx,tardias=tardias,faltas=faltas,meritos=meritos,amonestaciones=amonestaciones,ausencias=ausencias, direccion=direccion,mensaje = mensaje ,celular=celular,codigo=codigo,msk =msk , usuarioCompleto=usuarioCompleto)
    		else:
    ...
    def registrarf():
    	...
    	if request.method == "POST":
    		if request.form["acceso"] == 'ecom':
    			...
    			time.sleep(.6)
    			try:
    				new_user = UsuariosOficiales(username=request.form["username"],password=hashed_pw, nombreusuario= request.form["nombreusuario"],   codigo=request.form["codigo"],celular=request.form["celular"],direccion=request.form["direccion"])
    ...
    def horaentradaf():
    	...
    	if request.method == 'POST':
    		...
    		try:
    			...
    			try:
    			    OficialesEntrada(idoficial=idoficial, codigo=codigo, horaEntrada=horaEntrada, horaSalida=horaSalida,puesto= puesto)
    			    db.session.commit()
    ...
    def login():
    	if request.method == "POST":
    		...
    		password_condominio = request.form["password"]
    		time.sleep(2)
    		user = UsuariosSanAgustin.query.filter_by(username=request.form["username"]).first()
    		if user and check_password_hash(user.password, request.form["password"]):
    			...
    			usuarioCompleto = [[id_condominio, usuario_condominio,password_condominio, nombreusuario_condominio, pin_condominio,casa_condominio, permanentes_condominio ]]
    			time.sleep(1)
    ...
    def registrar():
    	...
    	if request.method == "POST":
    		if request.form['acceso'] == 'ecom':
    			...
    			time.sleep(.6)
    			try:
    				new_user = UsuariosSanAgustin(username=request.form["username"],password=hashed_pw, nombreusuario= request.form["nombreusuario"],   pin=request.form["pin"],casa=request.form["casa"],permanentes=request.form["permanentes"])
    				db.session.add(new_user)
    				db.session.commit()
    				mensaje ='Te has registrado con exito'
    			except Exception as e:
    ...
    def registrar_visita_diaria_san_agustin():
    	...
    	try:
    		if request.method == "POST":
    			...
    			fecha= MisHerramientas.mi_fecha_uno()
    			time.sleep(.6)
    			new_user = VisitasDiariasDB(idcondomino=idactual, nombrevisita= request.form["nombrevisita"],   empresa=request.form["empresa"],hora=request.form["horahora"],fecha=fecha)
    			db.session.add(new_user)
    			db.session.commit()
    ...
    def actualizar_perfil_condomino():
    	...
    	if request.method == "POST":
    		...
    		hashed_pw = generate_password_hash(request.form["passwordk"], method="sha256")
    		passwordk = hashed_pw
    		...
    		actualizar_perfil_app = (sessionx.query(UsuariosSanAgustin).filter_by(id=idk).first())
    ...
    def registrar_rancho_casa():
    	...
    	try:
    		if request.method == "POST":
    			...
    			time.sleep(.6)
    			new_user = Ranchos(idcondomino=idactual, etapa= request.form["etapa"],   hora=request.form["horahora"], fecha=fecha)
    			db.session.add(new_user)
    			db.session.commit()
...
if __name__ == '__main__':
    app.run(debug=True, port = 2341, host= '0.0.0.0')